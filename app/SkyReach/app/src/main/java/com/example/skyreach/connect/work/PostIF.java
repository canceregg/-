...
public class PostIF
{
    ...
    //返回：修改情况
    //删除帖子
    //参数：被删除帖子的postId、boardId
    //返回：修改情况
    public Boolean delPost(String postId, String boardId)
    {
    ...
    //增加帖子的点赞数加一
    //参数：postId->板块ID
    public Boolean decreaseLikeCnt(String postId)
    {
        final String url = "http://111.230.183.219/myapp2/PostLikeMinusServlet";
        result = MyHttp.getInstance().postSync(url,"postId", postId);
        System.out.println("输出-------:" + result);
        if(result.trim().equals("true"))
            return true;
